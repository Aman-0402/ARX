...
Every public page sets its own title/description/canonical/OG/Twitter tags
(and JSON-LD where relevant — `Organization` sitewide, `BlogPosting` per
post) via `frontend/src/components/SEO.jsx` (`react-helmet-async`).
`frontend/index.html` carries static fallback meta tags for crawlers/bots
that don't execute JS. `/robots.txt` and `/sitemap.xml` are generated by
Django (`config/urls.py`) — the sitemap includes every published blog post,
not just the fixed pages.
