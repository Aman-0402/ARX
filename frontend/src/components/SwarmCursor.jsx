...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    // Project addition: `global` mode listens on window instead of the
    // container so this can sit as a fixed, pointer-events:none overlay
    // spanning every page without swallowing clicks on real page content.
    const eventTarget = propsRef.current.global ? window : container;
    eventTarget.addEventListener('pointermove', onMove, { passive: true });
    eventTarget.addEventListener('pointerenter', onMove, { passive: true });
    ...
    return () => {
      ...
      eventTarget.removeEventListener('pointerenter', onMove);
